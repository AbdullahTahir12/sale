<?php
	...
	if (isset($_REQUEST['command']) && $_REQUEST['command'] == 'getWalkInSubLedgers' && intval($_REQUEST['id']) > 0) {
		...
		foreach ($sub_ledgers as $sub){
		 	$sub_ledger_op.='<option value="'.$sub['id'].'">'.$sub['name'].'</option>';
		}
		echo $sub_ledger_op;
		exit();
	}
	...
	if(isset($_REQUEST['command']) && $_REQUEST['command'] == 'GetDistributers'){
		...
		foreach ($ledgers as $ledger){
		 	$html.='<option data-active-walk-in="'.$ledger['active_walk_in_customer'].'" value="'.$ledger['id'].'">'.$ledger['name'].'</option>';
		}
		echo $html ;
		exit();
	}
	...
	if(isset($_REQUEST['command']) && $_REQUEST['command'] == 'getArticles'){
		...
		if($_REQUEST['type']=='journal'){
			...
			$db->Select("select ar.* from s_contract_inventory as sci ,s_contract_transaction as sct,sale_contract as sc ,sub_item as sb,article as ar  where sci.contract_id = sc.id and sci.c_transaction_id = sct.id and sci.sub_item_id = sb.id and sci.article_id = ar.id and sc.contract_type = '".$type."' and sc.customer_id = ".$customer." and sct.brand_id = ".$brand." and sb.code='".$code."' and sct.quality_id = ".$quality." and sct.item_id = ".$item_id." and sc.location_id = ".$location." and sc.sub_location_id = ".$sub_location." and sc.locked='no'");
		}
...
<body class="skin-green-light sidebar-mini">
						...
						<div class="box box-danger">
							<div class="box-body">
								...
								<div class="col-sm-12 no-gutter">
											...
											<div class="col-md-7 customer_div" <?php if($pi_row['sale_type'] == 'company_sale'){ ?>style="display: none"<?php } ?>>
												...
												<div class="form-group col-md-4 col-xs-12 no-gutter" style="padding-left: 0PX">
													...
													<select class="form-control chosen-transporter inv_transporter_id" name="customer_id" required="true" onchange="checkWalkInCustomer(this)">
														...
														?>
														<option data-active-walk-in="<?php echo $transporter['active_walk_in_customer']; ?>" <?php echo $select; ?> value="<?php echo $transporter['id']; ?>"> <?php echo $transporter['name']; ?></option>
														<?php } ?>
													</select>
												</div>
												<div class="col-md-4 walk_in_button_show" <?php if($pi_row['sub_ledger_id'] <= 0 || empty($pi_row)){ ?> style="display:none" <?php } ?> >
													...
													<div class="input-group">
														<select class="chosen-select walk_in" name="walk_in">
															...
															?>
															<option <?php if($s_ledger['id'] == $pi_row['sub_ledger_id']){ echo "selected";} ?> value="<?php echo $s_ledger['id']; ?>"><?php echo $s_ledger['name']; ?></option>
															<?php 
															}
															?>
														</select>
										...
										<!-- </div> -->
										<div class="col-sm-3 <?php if(!empty($pi_row)){echo 'hidden';} ?> " style="float:right">
								...
								<div class="clearfix" style="margin-bottom: 15px"></div>
																								...
																								<div class="input-group table-inputs">
																									...
																									<span class="pd_right input-group-addon no-gutter">
																										<select name="Sub_item_unit<?php echo $sr_no; ?>[]" onchange="QuantityValue('<?php echo 'SubItemModal'.$sr_no;?>',this)"class="showQ form-control" style="width: auto">
																											<option <?php if($subItem_inventory['unit'] == 'sqm'){echo "selected";} ?> value="sqm">Sqm</option>
																											<option <?php if($subItem_inventory['unit'] == 'boxes'){echo "selected";} ?> value="boxes">Boxes</option>
																											<!-- <option <?php if($subItem_inventory['unit'] == 'ton'){echo "selected";} ?> value="ton">ton</option> -->
																										</select>
																									</span>
																								</div>
																							</td>
												...
												<div class="chosenWidth chosen">
														<div class="form-group">
															<select class="form-control chosen-select " name="finishing_type[]" placeholder=" Enter Finishing Type" >
																<option> Select Finishing Type</option>
																<?php
																	$db->select('SELECT * FROM finishing_type where company_id='.getCompanyId());
									...
									<div class="main_button" style="text-align:right; padding-bottom: 10px ;margin: 10px">
										<input type="submit" name="purchaseInvoice" value="Save Sale Invoice" class="btn btn-primary">
	...
	function ShowSubItemModal(val_this) {
		...
		function getArticles(val_this,type){
			...
			$.ajax({
				url:'add-sale-invoice.php',
				data : {code,item,brand,project_id,sale_type,customer,quality,type,location_id,command:'getArticles'},
				type : 'POST',
				success : function(data){
		...
		function GetContractSubitems(val_this){
			...
			$.ajax({
				url : '<?php echo ADMIN_URL?>ajax/get-item-specifications.php',
				type : 'post',
				data : {sale_type,item_id,brand,quality,customer,project_id,location_id,command:'getContractSubitems'},
	...
	function ChangeSubItemModalNAmes(modal_no,sr){
		...
		function calculate(val){
			...
			if ($(val).closest('tr').find('.discount_type').val() == 'Rs'){
				...
				$(val).closest('tr').find('.amount').val(Math.floor(rem_amount));
		...
		function calculate_balance(val){
			...
			if ($(val).closest('tr').find('.discount_type').val() == '%') {
				...
				amount = net_rate * qty;
				total_discount = total_amt - amount;
				$(val).closest('tr').find('.amount').val(Math.floor(amount));
				$(val).closest('tr').find('.hidden_discount').val(total_discount);
			}else{
				rate = parseFloat($(val).closest('tr').find('.rate').val());
				discount =  parseFloat($(val).closest('tr').find('.discount').val());
		...
		function GetCustomers(){
			$.ajax({
				url : 'add-sale-invoice.php',
				datatype:'post',
				data : {data:$(".sale_type").val(),head:$(".location_id").val(),sub_head:$(".sub_location").val(),command:"GetDistributers"},
