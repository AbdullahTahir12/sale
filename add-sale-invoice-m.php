<?php
	...
	$auth_id=$a->authenticate();
	$user_row=$db->fetch_array_by_query("select * from admin where id=".$auth_id);
	...
	if(isset($_REQUEST['command']) && $_REQUEST['command'] == 'getArticles'){
		...
		$db->Select("SELECT ar.* FROM quotation_inventory as qi right join article ar on ar.id = qi.article_id right join sub_item as sb on qi.sub_item_id = sb.id  where qi.quality_id=".$quality." and qi.item_id=".$item_id." and sb.code='".$code."' and sb.brand_id = ".$brand." and location_id = ".$location." and sub_location_id= ".$sub_location." group by qi.article_id");
	...
	if(isset($_REQUEST['command']) && $_REQUEST['command'] == 'get_sub_loc') {
		$db->select('select * from item_sublocation where location_id='.$_REQUEST['item_loc'].' and id in('.$user_row["subhead_id"].')');
	...
	if(isset($_POST['purchaseInvoice'])){
		...
		if($receipt_id){
			...
			$db->Select("select * from sale_invoice where customer_id = ".$_REQUEST['customer_id']);
			$sales = $db->fetch_all();
			foreach($sales as $s){
				if($s['id'] == $receipt_id){
					continue;
...
.walk_in_button_show,.walk_in_text_show{
	margin-top: 25px
}
										...
										<div class="form-group">
											...
											<select class="transporter location_select proj_location chosen-select location_id" onchange="get_sub_location(this)" name="cr_location" id="location_id" required="true">
												<option selected>Select Location</option>
													<?php
													$db->select('SELECT * FROM item_location where id IN('.$user_row["head_id"].') order by id desc');
													$locationResults = $db->fetch_all();
													if(count_head($user_row["head_id"])==1 && count_subhead($user_row["subhead_id"])==1){
														foreach ($locationResults as $locationResult){
													 ?>
													 <option  value="<?php echo $locationResult['id'] ?>" selected><?php echo $locationResult['name'] ?></option>
													<?php 
												        }
													}else{
													foreach ($locationResults as $locationResult){
									...
									<div class=" col-lg-2 col-md-2 location_div">
										<div class="form-group">
											...
											<select name="cr_sublocation" class=" form-control chosen-select sub_location" id="sub_location_id" style="z-index:99999">
												<?php 
												if(count_head($user_row["head_id"])==1 && count_subhead($user_row["subhead_id"])==1){
													$db->Select("select * from item_sublocation where id in(".$user_row['subhead_id'].")");
													$sublocations = $db->fetch_all();
													foreach($sublocations as $sloc){
												?>
													<option value="<?php echo $sloc['id'];?>" selected><?php echo $sloc['name'];?></option>
												<?php
													}
												}else{
												?>
												...
												<?php
												$sql = 'SELECT * FROM item_sublocation where FIND_IN_SET('.getCompanyId().',company_id)';
												if($pi_row !=''){
													$sql .=' and location_id='.$pi_row['location_id'];
												}else{
													$sql .=' and location_id=59 ';
												}
									...
									<div class="col-lg-2 col-md-2 no-gutter">
										...
										<div class="col-lg-10 col-md-10 sales_person_div">
											<div class="form-group">
												...
												<select disabled class="form-control chosen-transporter sales_person" name="sales_person" required="true" onchange="checkWalkInCustomer(this)">
													<?php
										...
										<div class="col-md-9" style="padding-left:0">
											<div class="col-md-2 ">
												...
												<div class="form-group">
													...
													<select name="sale_type" onchange="CompanyTransactions(this)" class="form-control chosen-select sale_type">
														<?php if($auth_row['direct_sale'] == 'yes'){ ?> 
														<option value="customer">Retail</option>
														<?php } ?>
														<?php if($auth_row['whole_sale'] == 'yes'){ ?> 
														<option value="whole_sale">Whole Sale</option>
														<?php } ?>
														<?php if($auth_row['superadmin'] == 'yes'){?>
															<option value="whole_sale">Whole Sale</option>
															<option value="customer">Retail</option>
											...
											<div class="col-md-7 customer_div">
												...
												<div class="form-group col-md-4 col-xs-12 no-gutter" style="padding-left: 0PX">
													...
													<select class="form-control chosen-transporter inv_transporter_id" name="customer_id" required="true" onchange="checkWalkInCustomer(this)">
														...
														?>
														<option <?php echo $select; ?> value="<?php echo $transporter['id']; ?>"> <?php echo $transporter['name']; ?></option>
														<?php } ?>
													</select>
												</div>
												<div class="col-md-3" style="display:none" >
													<label>&nbsp</label>
													<button type="button" data-toggle="modal" data-target="#sub_ledger_model"  class="walk_in_button_show btn btn-primary btn-sm">show Walk In</button>
												</div>
												<div class="col-sm-2 text-left no-gutter" style="display:none" >
													<p class="walk_in_text_show" style="margin-left: 8px;padding-top: 10px;margin-bottom: 0;"></p>
												</div>
											</div>
											...
											<div class="col-md-7 company_sale_div hidden ">	
												<h4 style="border-bottom: 3px solid #dd4b39;width:fit-content;"> Debit : </h4>
												<div class="col-md-4 no-gutter">
													<div class="form-group col-xs-12 no-gutter" style="padding-left: 0PX">
														<label>Project </label>
														<select class="form-control chosen-select inv_transporter_id" 
														 name="dr_project" required="true">
															<?php
												...
												<div class="single-party-div col-md-8">
													<div class="form-group col-md-6 col-xs-6">
														...
														<select class="form-control chosen-select debit_head" onchange="getDebitSubLoc(this)" name="debit_head" required="true">
															...
															foreach ($locs as $loc){
															?>
															<option value="<?php echo $loc['id']; ?>" <?php echo $select;?>> <?php echo $loc['name']; ?></option>
															<?php } ?>
														</select>
													</div>
										...
										<table class="table table-striped pq_table">
											<thead>
												<tr class="title_bg">
													...
													<th colspan="2" class="border-b0">Rate</th>
													<th rowspan="2" class="hidden discount_heading border-b0">Discount</th>
											...
											<tr class="pq_row">
																				...
																				<table class="table table-striped SubItemTable">
																					<thead>
																						<tr>
																							...
																							<th class="quality_td" <?php if($checkItem['quality'] == 'no'){ echo 'style="display:none"';}?>>Quality</th>
																					...
																					<tbody class="SubItemBody">
																						...
																						<tr class="SubItemRow">
																							...
																							<td>
																								...
																								$db->select("select * from sub_item where item_id=".$purchaseDetail['item_id']);
																								$sub_items = $db->fetch_all();
																								?>
																								<div class="form-group">
																									<select  onchange="CheckSubItem(this)"  class=" SubItemCode chosen-select chosen-code  form-control" >
																										<option value="0"> Select Subitem code</option>
																										<?php
																										foreach($sub_items as $sub ){
																											$select = '';
																											if($subItem_inventory['sub_item_id'] ==  $sub['id']){
																												$select = 'selected';
																											}
																										?>
																							...
																							<td class="quality_td" <?php if($checkItem['quality'] == 'no'){echo 'style="display:none"';}?>>
																								<div class="form-group">
																									<?php
																							...
																							<td>
																								...
																								<div class="input-group table-inputs">
																									<input type="number" onkeyup="QuantityAdd(this,'<?php echo 'SubItemModal'.$sr_no;?>')" class="form-control inv_quantity" value="<?php echo $subItem_inventory['inv_quantity'];?>" name="Sub_item_quantity<?php echo $sr_no; ?>[]" placeholder="Enter Quantity">
																									<span class="pd_right input-group-addon"><b class="qty_symbol"><?php echo $itemUnit['symbol']; ?></b></span>
																								</div>
																								<div style="margin-top: 10px"><span class="alterNateUnitSpan"><?php echo getalternateUnitSpan($subItem_inventory['inv_quantity'],$checkItem['id']);?></span></div>
																							</td>
																							<td>
																								<div class="input-group">
																									<?php
																									$avail_q = GetSubitemQuantity( $subItem_inventory['location_id'],$subItem_inventory['sub_location_id'],$subItem_inventory['sub_item_id'],$subItem_inventory['article_id'],$subItem_inventory['quality_id'],'','');
																									?>
																									<input type="number" class="form-control available_quantity" value="<?php echo $avail_q;?>" readonly >
																									<span class="input-group-addon qty_symbol"><?php echo $itemUnit['symbol']; ?></span>
																								</div>
																							</td>
																							<td>
																								<button class="btn btn-primary" onclick="AddSubItemRow(this)" type="button"><i class="fa fa-plus-circle"></i></button>
																								<button class="btn btn-danger" onclick="removeSubItemrow(this,'<?php echo 'SubItemModal'.$sr_no;?>')" type="button"><i class="fa fa-minus-circle"></i></button>
																							</td>
																						</tr>
																						<?php } ?>
																					</tbody>
																					<tfoot>
																						<tr>
																							<td></td>
																							<td></td>
																							<td  <?php if($checkItem['article'] == 'no'){ echo 'style="display:none"';}?>></td>
																							<td class="quality_td" <?php if($checkItem['quality'] == 'no'){ echo 'style="display:none"';}?>></td>
																							<td class="color_td" <?php if($checkItem['color'] == 'no'){ echo'style="display:none"';}?> ></td>
																							<td class="quantity_td"><span class="quantity_span">Total Quantity : <b class="quantity_b"><?php echo $total_quantity;?></b></span> <input type="hidden" value="<?php echo $total_quantity;?>" class="total_auantity" ></td>
																							<td></td>
																						</tr>
																					</tfoot>
																				</table>
												...
												<td>
													<div class="input-group table_inputs">
														...
														<span class="pd_right input-group-addon"><b class="qty_symbol"><?php if($itemSymbol['symbol']!=''){echo $itemSymbol['symbol'];
														}else{
														echo "unit";}?></b></span>
													</div>
													<div style="margin-top: 10px"><span class="alterNateUnitSpan"><?php echo getalternateUnitSpan($purchaseDetail['quantity'],$checkItem['id']);?></span></div>
													<!-- Receipt Toggle -->
													<div class="invoice_md_button">
													</div>
												</td>
												<td class="mat_modal">
													<div class="row">
														...
														<p> eff Rate : <span class="eff_net_rate_area"> <?php echo $purchaseDetail['effective_rate'] ?></span></p>
														<input type="hidden" name="net_rate[]" class="net_rate net_rate_area" value="<?php echo $purchaseDetail['net_rate'] ?>">
														<input type="hidden" name="eff_net_rate[]" class="eff_net_rate" value="<?php echo $purchaseDetail['effective_rate'] ?>">
													</div>
												...
												<td class="enable_multiple_discount">
													<div class="input-group table_inputs">
														<input type="number" name="discount[]" class="form-control discount" onkeyup="calculate_discount(this)" placeholder=" Enter Discount " required="true" value="<?php echo $purchaseDetail['discount']; ?>">
														<span style="padding: 0px;" class="pd_right input-group-addon">
															<select onchange="calculate(this)" class="discount_type form-control" name="balance_category[]" style="width: auto;">
																...
																<option <?php if($purchaseDetail['balance_category']=="%"){ echo "selected";} ?>  value="%">%</option>
															</select>
														</span>
													</div>
											...
											<?php }
												...
												<td>
													...
													<td>
														...
														<input type="number" placeholder="Effective Rate" class="form-control eff_net_rate" name="eff_net_rate[]" readonly>
													</td>
...
<!-- 														<div class="hidden_disc_amt"></div> -->
											...
											<tfoot>
												<tr class="footer_bg">
													<td></td>					
													<td style="text-align: center;" colspan="4"> Gross Total </td>
													...
													<td class="tot_boxes">0</td>
													<td class="tot_tons hidden">0</td>
													<td></td>
													<td></td>
													<td class="discount_heading hidden"><span id="total_discount_txt">0</span></td>
									...
									<div class="clearfix"></div>
									<div class="main_button col-sm-6 pull-right" style="text-align:right; padding-bottom: 10px ;margin: 10px">
										<div class="col-sm-3"></div>
										<div class="form-group col-sm-4 pull-left text-left ">
										...
										<div class="form-group col-sm-5 pull-right text-left no-gutter" style="margin-top: 5px">
											<br>
											<input type="submit" name="purchaseInvoice" value="Save Sale Invoice" class="pull-left btn btn-primary">
											<button type="button" class="btn btn-danger pull-right"> Close Sale Invoice </button>
										</div>
									</div>
								</div>
 								</div>
	...
	function ChangeSubItemModalNAmes(modal_no,sr){
		...
		function checkContract(val_this){			
			...
			method: 'POST',
			data: {item_id,project_id,brand,sale_type,quality,customer,location_id}
			}).done(function(result){
				res = $.parseJSON(result);
		...
		function calculate(val){
			...
			if ($(val).parent().parent().parent().find('.discount_type').val() == 'Rs'){
				...
				$(val).parent().parent().parent().find('.hidden_disc_amt').text('');
		...
		function calculate_quantity(val_this){
		 ...
		 // 	if(alternate_unit !='' && conversion !=0){
			...
			// 	}
			// 	text +=" )"
			// 	$(val_this).parent().parent().find(".alterNateUnitSpan").text(text).parent().css({"margin-top":"10px"});
			// }
		 	$('#discount').trigger('onkeyup');
		}
		...
		function calculate_balance(val){
			...
			if ($(val).parent().parent().parent().find('.discount_type').val() == '%') {
				...
				net_rate = rate - percentage;
				individual = rate - net_rate;
				$(val).parent().parent().parent().find(".individusl_discount span").text(individual);
		...
		function calculateTotalDiscount(){
			...
			$("#total_discount_txt").text(total_discount);
		}
			...
			$.ajax({
				...
				success : function(response){
					if (response.status=='yes') {
						...
						$('.walk_in_button_show').parent().show();
					}else{
						$('.walk_in_button_show').parent().hide();
					}
				}
			})
		}
