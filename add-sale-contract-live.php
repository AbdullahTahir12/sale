<?php	
	...
	if(isset($_POST['purchaseContract'])){
		...
		$sql .= "select * from sale_contract where contract_type = '".$_POST['contract_type']."' and location_id=".intval($_POST['location'])." and sub_location_id=".intval($_POST['sub_location']).' and company_id ='.getCompanyId();
		if(isset($_POST['customer']) && intval($_POST['customer']) > 0){
			$sql .=' and FIND_IN_SET(customer_id,"'.implode(',',$_POST['customer']).'")';
		}

		if(isset($_POST['subledger']) && intval($_POST['subledger']) > 0){
			$sql .=' and FIND_IN_SET(subledger_id,"'.implode(',',$_POST['subledger']).'")';
		}

		$sql .=' order by id desc';
		$db->Select($sql);
		$last_contracts = $db->fetch_all($sql);
		if($last_contracts){
								...
								<div class="row" style="padding: 13px">
									<div class="col-sm-12">
										...
										<div class="col-md-6 no-gutter">
											...
											<div class="col-sm-2">
											<div>&nbsp;</div>
											<label style="margin-top: 15px"><input <?php if($contract['subitem_mode'] == 'yes'){ echo "checked";}?> type="checkbox" class="subitem_mode" name="subitem_mode" value="yes"> Subitem </label>
										</div>
									...
									<div class="col-md-12">
										<div class="col-md-6 no-gutter">
											...
											<div class="col-sm-2 customer-mode" <?php if($contract['contract_type'] == 'distributer' || $contract['contract_type'] == 'project' ){ ?> style="display: none"<?php } ?>>
												...
												<label style="margin-top: 9px">
														<input type="checkbox" <?php if($contract['customer_mode'] == 'yes'){echo "checked";}?> class="customer-check" name="customer_mode" onclick="ContractType(this)" value="yes">&nbsp;Customer
												</label>
											</div>
											<div class="col-md-5 customer <?php echo $s_contract['contract_type']; ?>" <?php if($s_contract['contract_type'] == 'whole_sale' && $s_contract['customer_mode'] == 'yes' || $s_contract['contract_type'] == 'company_sale' && $s_contract['customer_mode'] == 'yes'){?> style="display: none"<?php } ?>>
												...
												<select class="chosen-customer " multiple="" style="width: 100% !important" onchange="get_sub_ledger(this)" name="customer[]">
													...
													?>
													<option <?php if(in_array($c['id'],$c_customers)){ echo "selected";}?> value="<?php echo $c['id']?>"><?php echo $c['name']?></option>
													<?php } ?>
												</select>
											</div>
										</div>
										<div class="col-md-6 no-gutter">
											...
											<div class="col-sm-2">
												<div>&nbsp;</div>
												<label style="margin-top: 9px"><input <?php if($contract['discount_mode'] == 'yes'){ echo "checked";}?> type="checkbox" name="discount_mode" value="yes"> Discount </label>
											</div>
										</div>
									...
									<table class="table table-striped pq_table">
										<thead>
											<tr class="title_bg">
												...
												<th colspan="2" style="width:25%" class="<?php if($contract['discount_mode'] == 'no'){ echo "hidden";}else if(empty($contract)){echo "hidden";} ?> discount_heading border-b0">Discount</th>												
												<th rowspan="2" style="width:5%">Action</th>
											</tr>
											<tr class="<?php if($contract['discount_mode'] == 'no'){ echo "hidden";}else if(empty($contract)){echo "hidden";} ?> title_bg discount_heading">
										...
										<tbody class="pq_body">
											...
											foreach ($p_t_contracts as $p_t_contract){
																				...
																				<div class="table-responsive">
																					...
																					<table class="table table-striped table-bordered SubItemTable">
																						...
																						<tbody class="SubItemBody">
																							<?php 
																							$db->Select("select sb.* from sub_item as sb right join quotation_inventory as pii on (pii.item_id = sb.item_id and pii.quality_id = ".$p_t_contract['quality_id'].") where sb.item_id=".$p_t_contract['item_id']." and sb.brand_id = ".$p_t_contract['brand_id']." and sb.company_id=".getCompanyId()." and pii.location_id = ".$contract['location_id']." and pii.sub_location_id = ".$contract['sub_location_id']." group by sb.id");
																							$subitems = $db->fetch_all();
																							$no=0;
																							foreach($subitems as $inv){
																							
																								$contract_inventory = $db->fetch_array_by_query("select * from s_contract_inventory where contract_id = ".$contract['id']." and c_transaction_id = ".$p_t_contract['id']." and sub_item_id = ".$inv['id']);
                                                ...
                                                <td class="enable_multiple_discount <?php if($contract['discount_mode'] == 'no'){ echo "hidden";} ?>">
													<div class="input-group" >
														...
														<span class="input-group-addon no-gutter">
															<select style="padding: 5px" name="discount_type[]" onchange="calculate_net(this)" class="discount-type">
																<option value="Rs" <?php if($p_t_contract['discount_type'] == 'Rs'){echo "selected";}?>>No.</option>
																<option value="%" <?php if($p_t_contract['discount_type'] == '%'){echo "selected";}?> >%</option>
															</select>
														</span>
													</div>
												</td>
                                                <td  class="multiple_discount <?php if($contract['discount_mode'] == 'no'){ echo "hidden";} ?>">
                                                	<div class="form-group">
										...
										<tfoot>
											<tr class="footer_bg">
												...
												<td class="discount_heading <?php if($contract['discount_mode'] == 'no'){ echo "hidden";}else if(empty($contract)){echo "hidden";} ?>"></td>
												<td class="discount_heading <?php if($contract['discount_mode'] == 'no'){ echo "hidden";}else if(empty($contract)){echo "hidden";} ?>"></td>
												<td></td>
												<td></td>
											</tr>
				...
				success : function(result){
					...
					if(result == 'yes'){
						...
						$.ajax({
							...
							type: "POST",
							data: {sr_no,item_id,brand_id,quality_id,project_id,location_id},
						}).done(function(result) {
                        	$(".SubItemModal"+sr_no).find(".SubItemTable tbody").html(result);  
                        	$(".SubItemModal"+sr_no).find(".SubItemTable tbody .chosen-select").chosen({width:'100%'});
	...
	function getDelivery(val_this){
		...
		var selected_type=$(val_this).val();
                ...
                // return;
                    ...
                    url: "add-sale-contract.php",
                    method: "POST",
                    data: {selected_type,location_id,sublocation_id,command},
                    success : function(result){
				console.log(result);
