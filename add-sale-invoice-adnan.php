 <?php
	...
	// 	global $auth_row;
	// 	if('super_admin' == 'yes'){
	// 		$db->select('select * from item_location where locked= "no" and FIND_IN_SET('.getCompanyId().',company_id)');
	// 	}else{
	// 		$db->select('select * from item_location where locked= "no" and FIND_IN_SET(id,"'.$auth_row['head_id'].'")');
	// 	}
	//     $rzlt = $db->fetch_all();
	// 	return($rzlt);
	// }
	...
	function getBalance($id, $type, $days = '')
	{
		global $db;
		if ($days != '') {
		...
		if ($type == 'ledger') {
			...
			$sql .= "SELECT SUM(case when tr.type='credit' then tr.amount else 0 end) as credit, SUM(case when tr.type='debit' then tr.amount else 0 end) as debit from transactions as tr right join voucher as vc on (vc.id = tr.voucher_id) where tr.ledger_id=" . $id;
			if ($days != '') {
				$sql .= " and DATE_FORMAT( DATE_ADD(DATE_FORMAT(FROM_UNIXTIME(vc.date), '%Y-%m-%d %H:%i:%s'), INTERVAL 5 HOUR), '%Y-%m-%d ') >='" . date_format($date, "Y-m-d") . "' and DATE_FORMAT( DATE_ADD(DATE_FORMAT(FROM_UNIXTIME(vc.date), '%Y-%m-%d %H:%i:%s'), INTERVAL 5 HOUR), '%Y-%m-%d ')<='" . $current_date . "'";
			}
			$tr_row = $db->fetch_array_by_query($sql);
		} else {
			$ledger_row = $db->fetch_array_by_query("select * from sub_ledgers where id=" . $id);
			$sql .= "SELECT SUM(case when tr.type='credit' then tr.amount else 0 end) as credit, SUM(case when tr.type='debit' then tr.amount else 0 end) as debit from transactions as tr right join voucher as vc on (vc.id = tr.voucher_id) where tr.sub_ledger_id=" . $id;
			if ($days != '') {
				$sql .= " and DATE_FORMAT( DATE_ADD(DATE_FORMAT(FROM_UNIXTIME(vc.date), '%Y-%m-%d %H:%i:%s'), INTERVAL 5 HOUR), '%Y-%m-%d ') >='" . date_format($date, "Y-m-d") . "' and DATE_FORMAT( DATE_ADD(DATE_FORMAT(FROM_UNIXTIME(vc.date), '%Y-%m-%d %H:%i:%s'), INTERVAL 5 HOUR), '%Y-%m-%d ')<='" . $current_date . "'";
			}
			$tr_row = $db->fetch_array_by_query($sql);
	...
	if (isset($_REQUEST['command']) && $_REQUEST['command'] == 'CheckOverDue') {
		$o_id = $_REQUEST['o_id'];
		$days = $db->fetch_array_by_query("Select * from outstanding_days where id = " . $o_id);
		$outstanding_days = $days['value'] . ' ' . $days['o_interval'];
		$ledger = $_REQUEST['customer'];
		if (intval($_REQUEST['sub_ledger']) > 0) {
			$row = $db->fetch_array_by_query("Select * from sub_ledgers where id = " . $_REQUEST['sub_ledger']);
		} else {
			$row = $db->fetch_array_by_query("Select * from ledger where id = " . $ledger);
		}
		$sub_ledger = $_REQUEST['sub_ledger'];

		if (intval($_REQUEST['sub_ledger']) > 0) {
			$balance = getBalance($sub_ledger, 'sub_ledger', $outstanding_days);
		} else {
			$balance = getBalance($ledger, 'ledger', $outstanding_days);
		}
		$balance['days'] = $outstanding_days;
	...
	if (isset($_REQUEST['command']) && $_REQUEST['command'] == 'getWalkInSubLedgers' && intval($_REQUEST['id']) > 0) {
		...
		foreach ($sub_ledgers as $sub) {
			$sub_ledger_op .= '<option data-o-days="' . $sub['outstanding_days'] . '" data-credit="' . $sub['credit_limit_active'] . '" data-cvalue="' . $sub['credit_limit'] . '" value="' . $sub['id'] . '">' . $sub['name'] . '</option>';
		}
		echo $sub_ledger_op;
		exit();
	}
	...
	if (isset($_REQUEST['command']) && $_REQUEST['command'] == 'GetDistributers') {
		...
		if ($data == 'distributer') {
			...
			// echo "select * from ledger where active_distributer='yes' and FIND_IN_SET('" . $head . "',distributer_head) and FIND_IN_SET('" . $sub_head . "',distributer_subhead) and FIND_IN_SET(" . getCompanyId() . ",company_id)";
			// die();
		...
		foreach ($ledgers as $ledger) {
			...
			$html .= '<option ' . $select . ' data-o-days="' . $customer['outstanding_days'] . '" data-cvalue="' . $ledger['credit_limit'] . '" data-credit="' . $ledger['credit_limit_acitve'] . '" data-active-walk-in="' . $ledger['active_walk_in_customer'] . '" value="' . $ledger['id'] . '">' . $ledger['name'] . '</option>';
		}
		echo $html;
		exit();
	}
	...
	function getContractArticles($subitem, $purchaseDetail, $row, $sales_person)
	{
		global $db;
		...
		$customer_contract = $db->fetch_array_by_query("Select * from sale_contract where FIND_IN_SET(" . $row['customer_id'] . ",customer_id) and location_id = " . $purchaseDetail['location'] . " and project_id=" . $purchaseDetail['sub_location'] . " and locked ='no' and status = 'approved'");
		if ($row['sale_type'] != 'agent') {
			if ($customer_contract) {
				$sql .= ' and FIND_IN_SET(' . $row['customer_id'] . ',sc.customer_id)';
			}
			$subledger_contract = $db->fetch_array_by_query("Select * from sale_contract where  FIND_IN_SET(" . $row['customer_id'] . ",customer_id)  and  FIND_IN_SET(" . $row['sub_ledger_id'] . ",sub_ledger_id) and location_id = " . $purchaseDetail['location'] . " and contract_type = '" . $row['sale_type'] . "' and project_id=" . $purchaseDetail['sub_location'] . " and locked ='no' and status = 'approved'");
			if (!empty($subledger_contract) && $row['sub_ledger_id'] > 0) {
				$sql .= " and FIND_IN_SET(" . $row['sub_ledger_id'] . ",sc.sub_ledger_id)";
			}
		} else {
			$sql .= " and FIND_IN_SET(" . $sales_person . ",sc.sub_ledger_id)";
		}

		$sql .= " and find_in_set(" . $purchaseDetail['brand'] . ",sct.brand_id)";
		if (intval($purchaseDetail['finishing_type']) > 0) {
			$sql .= " and sct.finishing_type = " . $purchaseDetail['finishing_type'];
		}
		$sql .= " and sb.code='" . $subitem . "' and sct.quality_id = " . $purchaseDetail['quality'] . " and sct.item_id = " . $purchaseDetail['item_id'] . " and sc.location_id = " . $purchaseDetail['location'] . " and sc.sub_location_id = " . $purchaseDetail['sub_location'] . " and sc.branch_id = " . $purchaseDetail['branch_id'] . " and sc.locked='no' and  sc.status='approved'";
		$db->Select($sql);
	}
	...
	{
		global $db;
		$sql = "select sb.code from s_contract_inventory as sci ,s_contract_transaction as sct,sale_contract as sc ,sub_item as sb where sci.contract_id = sc.id and sci.c_transaction_id = sct.id and sc.branch_id='" . $row['branch_id'] . "' and sb.id = sci.sub_item_id and sc.contract_type = '" . $row['sale_type'] . "'";
		$customer_contract = $db->fetch_array_by_query("Select * from sale_contract where FIND_IN_SET(" . $row['customer_id'] . ",customer_id) and location_id = " . $purchaseDetail['location'] . " and project_id=" . $purchaseDetail['sub_location'] . " and locked ='no' and status = 'approved'");
		if ($row['sale_type'] != 'agent') {
			if ($customer_contract) {
				$sql .= ' and FIND_IN_SET(' . $row['customer_id'] . ',sc.customer_id)';
			...
			$subledger_contract = $db->fetch_array_by_query("Select * from sale_contract where  FIND_IN_SET(" . $row['customer_id'] . ",customer_id)  and  FIND_IN_SET(" . $row['sub_ledger_id'] . ",sub_ledger_id) and location_id = " . $purchaseDetail['location'] . " and contract_type = '" . $row['sale_type'] . "' and project_id=" . $purchaseDetail['sub_location'] . " and locked ='no' and status = 'approved'");
			// print_r($subledger_contract);
			if (!empty($subledger_contract) && $sub_ledger > 0) {
				$sql .= " and FIND_IN_SET(" . $sub_ledger . ",sc.sub_ledger_id)";
			}
		} else {
			$sql .= " and FIND_IN_SET(" . $row['sales_person'] . ",sc.sub_ledger_id)";
		}
		$sql .= " and FIND_IN_SET(" . $purchaseDetail['brand'] . ",sct.brand_id) and sct.quality_id = " . $purchaseDetail['quality'];
		if (intval($purchaseDetail['finishing_type']) > 0) {
			$sql .= " and sct.finishing_type = " . $purchaseDetail['finishing_type'];
		}
		$sql .= " and sct.item_id = " . $purchaseDetail['item_id'] . " and sc.branch_id = " . $purchaseDetail['branch_id'] . " and sc.location_id = " . $purchaseDetail['location'] . " and sc.sub_location_id = " . $purchaseDetail['sub_location'] . " and sc.company_id = " . getCompanyId() . " and sc.locked='no' and sc.status = 'approved' group by sb.code";
		$db->Select($sql);
	}
	...
	function GetSubitemQuantity($head, $sub_head, $sub_item_id, $article, $quality, $section, $warehoue_id, $branch, $brand_id, $finishing_type)
	{
		global $db;
		...
		foreach ($tables as $table) {
			...
			if (intval($finishing_type) != '') {
				$sql .= " and finishing_type = " . $finishing_type;
			}
			$sql .= " and location_id=" . $head . " and sub_location_id=" . $sub_head . " and branch_id = " . $branch . " and company_id=" . getCompanyId();
	...
	if (isset($_REQUEST['command']) && $_REQUEST['command'] == 'getQuantity') {
		...
		if (isset($_REQUEST['head']) && $_REQUEST['head'] != ''  && isset($_REQUEST['sub_head']) && $_REQUEST['sub_head'] != '') {
			...
			$sub_item_brand = $_REQUEST['sub_item_brand'];
			$finishing_type = $_REQUEST['finishing_type'];
			$quantity = GetSubitemQuantity($head, $subhead, $sub_item_id, $article, $quality, '', '', $branch, $sub_item_brand, $finishing_type);
		}
		echo $quantity;
		exit();
	}
	...
	if (isset($_POST['purchaseInvoice']) || isset($_POST['purchaseInvoiceApprove'])) {
		$arr = array();
		...
		if (isset($_POST['purchaseInvoiceApprove'])) {
			$arr['status'] = 'approved';
		}
		...
		if ($receipt_id) {
			...
			if ($_POST['item_name']) {
				...
				for ($i = 0; $i < count($_POST['item_name']); $i++) {
					...
					$arr_detail['branch_id'] = intval($_POST['branch']);
					...
					if ($_POST['sr_no'][$i] != 0) {
						...
						if (isset($_REQUEST['Sub_item_id' . $sr_no]) && $_REQUEST['Sub_item_id' . $sr_no] != '') {
							foreach ($_REQUEST['Sub_item_id' . $sr_no] as $subItemIndex => $subItemId) {
								...
								$receipt_inv['branch_id'] = intval($_POST['branch']);
	...
	if (isset($_REQUEST['command']) && $_REQUEST['command'] == 'sub_head_loc') {
		$db->select('select * from item_sublocation where location_id=' . $_REQUEST['loc_id'] . ' and FIND_IN_SET(' . getCompanyId() . ',company_id)');
		$sub_head_data = $db->fetch_all();
		$sub_head_location .= "<option selected disabled>Select Sub Location</option>";
		foreach ($sub_head_data as $sub_head) {
			$sub_head_location .= "<option value='" . $sub_head['id'] . "'>" . $sub_head['name'] . "</option>";
		}
		echo $sub_head_location;
		exit();
	}
 ...
 <body class="skin-green-light sidebar-mini">
 								...
 								<div class="box-body">
 									<div class=" lb_top">
 										...
 										<div class=" col-lg-2 col-md-2  location_div">
 											<div class="form-group">
 												<label> Head </label>
 												<select class="transporter location_select proj_location chosen-select location_id" name="cr_location" onchange="get_sub_head(this)" id="location_id" required="true">
 										...
 										<div class="col-lg-2 col-md-2 no-gutter">
 											<label> Branch </label>
 											<select class="chosen-select branch_val " name="branch" required="true">
 												<option selected>Select Branch</option>
 												<?php
													if (isset($_REQUEST['so_id']) && intval($_REQUEST['so_id']) > 0) {
														$db->select("Select * from branches where id = " . $pi_row['branch_id']);
													} else {
													...
													foreach ($branch_data as $branch) { ?>
 													<option value="<?php echo $branch['id'] ?>" <?php if ($pi_row['branch_id'] == $branch['id']) {
																										echo "selected";
 												...
 												<div class=" customer form-group col-md-4 col-xs-12 no-gutter" style="padding-left: 0PX">
 													...
 													<select class="form-control chosen-transporter customer_id inv_transporter_id" name="customer_id" required="true" onchange="checkWalkInCustomer(this)">
 														...
 														<?php
															...
															?>
 															<option data-o-days="<?php echo $transporter['outstanding_days']; ?>" data-credit="<?php echo  $transporter['credit_limit_acitve']; ?>" data-cvalue="<?php echo  $transporter['credit_limit']; ?>" data-active-walk-in="<?php echo $transporter['active_walk_in_customer']; ?>" <?php echo $select; ?> value="<?php echo $transporter['id']; ?>"> <?php echo $transporter['name']; ?></option>
 														<?php } ?>
 													</select>
 												</div>
 													...
 													<div class="input-group">
 														<select class="chosen-select walk_in" onchange="getBalance(this,'subledger')" name="walk_in">
 															...
 															<?php
																...
																?>
 																<option data-o-days="<?php echo $s_ledger['outstanding_days']; ?>" data-credit="<?php echo  $transporter['credit_limit_acitve']; ?>" data-cvalue="<?php echo  $s_ledger['credit_limit']; ?>" <?php if ($s_ledger['id'] == $pi_row['sub_ledger_id']) {
																																																																	echo "selected";
 											...
 											<div class="col-md-7 company_sale_div <?php if ($pi_row['sale_type'] != 'company_sale') { ?>hidden<?php } ?>  ">
 												...
 												<div class="single-party-div col-md-8">
 													...
 													<div class="form-group col-md-6 col-xs-6 no-gutter">
 														<label> Sub Head </label>
 														<select class="form-control chosen-select" name="debit_subhead" required="true">
 																										...
 																										<?php
																											...
																											?>
 																											<tr class="SubItemRow">
 																												...
 																												<td>
 																													...
 																													<div class="input-group table-inputs">
 																														...
 																														<span class="pd_right input-group-addon no-gutter">
 																															<select onchange="QuantityValue('<?php echo 'SubItemModal' . $sr_no; ?>',this)" class="showQ form-control" style="width: auto">
 																																<?php
																																	...
																																	foreach ($units as $unit) {
																																		...
																																		if ($subItem_inventory['unit'] == 'boxes') {
																																			if ($unit['name'] == 'box') {
																																				$select = 'selected';
																																			}
																																		}

																																		if ($subItem_inventory['unit'] == 'sqm') {
																																			if ($unit['symbol'] == 'sqm') {
																																				$select = 'selected';
																																			}
																																		}

																																		if ($subItem_inventory['unit'] == 'bags') {
																																			if ($unit['name'] == 'bag') {
																																				$select = 'selected';
																																			}
																																		}
 																														...
 																														<input name="Sub_item_unit<?php echo $sr_no; ?>[]" type="hidden" value="<?php echo $subItem_inventory['unit']; ?>" class="sub-item-unit">
 																													</div>
 																												</td>
 																												<td class="alternate_td">
 																													<?php
																														if (intval($checkItem['alternative_unit_id']) > 0) {
																															if ($subItem_inventory['unit'] == 'sqm') {
																																echo $subItem_inventory['inv_quantity'] . ' ' . $itemSymbol['symbol'] . ' (' . round($subItem_inventory['value'] / $checkItem['conversion'], 2) . ' ' . $alternate_unit['symbol'] . ')';
																															} else if ($subItem_inventory['unit'] == 'boxes') {
																																echo $subItem_inventory['inv_quantity'] . ' ' . $itemSymbol['symbol'] . ' (' . round($subItem_inventory['value'] * $checkItem['conversion'], 2) . ' ' . $itemSymbol['symbol'] . ')';
																															}
																														} else {
 																												...
 																												<td>
 																													<span style="font-weight: bold" class="available_quantity_boxes"><?php if (isset($checkItem['conversion']) and intval($checkItem['conversion']) > 0) {
																																															echo round($avail_q / $checkItem['conversion'], 2);
																																														} else {
																																															echo ' - ';
																																														} ?></span>
 																												</td>
 									...
 									<div class="container-fluid" style="padding: 20px">
 										...
 										<div class="clearfix"></div>
 										<div class="main_button col-sm-12" style=" padding-bottom: 10px ;margin: 10px">
 											<div class="col-md-9 no-gutter ">
 												<div class="form-group col-sm-2 no-gutter pull-right">
 											...
 											<div class="col-md-3 no-gutter" style="text-align:right;">
 												<div>&nbsp;</div>
 												<?php if (intval($pi_row['sub_ledger_id']) > 0) {
														$sub = $db->fetch_array_by_query("Select * from sub_ledgers where id = " . $pi_row['sub_ledger_id']);
														$type = 'sub_ledger';
													} else {
														$sub = $db->fetch_array_by_query("Select * from ledger where id = " . $pi_row['customer_id']);
														$type = 'ledger';
													}
													$days = $db->fetch_array_by_query("Select * from outstanding_days where id = " . $sub['outstanding_days']);
													$limit = getBalance($sub['id'], $type, $days['value'] . ' ' . $days['o_interval']);
													// print_r($limit);
													if ($limit['type'] == '(Cr)') {

													?>
 													<input type="submit" name="purchaseInvoice" value="Save Sale Invoice" class="btn add_btn btn-primary">
 												<?php } else if ($limit['type'] == '(Dr)' && $auth_row['super_admin'] == 'yes') { ?>
 													<input type="submit" name="purchaseInvoiceApprove" value="Save Sale Invoice & Approve" class="btn btn-primary">
 												<?php } else if (!isset($_REQUEST['so_id'])) { ?>
 													<input type="submit" name="purchaseInvoice" value="Save Sale Invoice" class="btn add_btn btn-primary">
 												<?php } ?>
 	...
 	<script type="text/javascript">
 		...
 		function checkSubItems(val_this, project_id, location_id) {
 			...
 			$.ajax({
 				url: 'add-sale-invoice-adnan.php',
 				Type: 'post',
 				data: {
 					item_id,
 		...
 		function chosenValue(val_this) {
 			...
 			if (project_id == "0" || project_id == null || isNaN(project_id)) {
 				...
 				$.ajax({
 					...
 					checkContract(val_this, 'contract');
 		...
 		function getBalance(val, type) {
 			...
 			$.ajax({
 				...
 				success: function(balance) {
 					...
 					if (type == 'subledger') {
 						CheckOverDue(val, 'subledger');
 					}
 				}
 			});
 		}

 		function CheckOverDue(val, type) {
 			if (type == 'ledger') {
 				var customer = $(val).val();
 				var o_id = $(val).find("option:selected").data('o-days');
 			} else {
 				var sub_ledger = $(val).val();
 				var o_id = $(val).find("option:selected").data('o-days');
 			}
 			if (parseInt(customer) > 0 || parseInt(sub_ledger) > 0) {
 				$.ajax({
 					url: 'add-sale-invoice-adnan.php',
 					type: 'post',
 					data: {
 						...
 						sub_ledger,
 						o_id,
 						command: 'CheckOverDue'
 					},
 					success: function(data) {
 						rate = $.parseJSON(data);
 						if (rate.type == '(Dr)') {
 							alert("No Credit During last " + rate.days);
 							$(".add_btn").hide();
 						} else if (rate.type == '(Cr)') {
 							$(".add_btn").show();
 						} else {
 		...
 		function get_sub_head(val_this) {
 			$.ajax({
 					url: 'add-sale-invoice-adnan.php',
 					type: 'POST',
 					data: {
 						loc_id: $(val_this).val(),
 						command: 'sub_head_loc'
 					},
 				})
 				.done(function(data) {
