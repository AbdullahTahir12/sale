<?php
	...
	// function InvoiceNo(){
	// 	global $db;
	// 	$last_record = $db->fetch_array_by_query("SELECT * FROM sale_invoice WHERE company_id=". getCompanyId()." ORDER BY ID DESC");
	// 	$number = substr($last_record['sale_invoice_no'], 4);
	// 	$month_arr = str_split($last_record['sale_invoice_no']);
	// 	$month = $month_arr[2]."".$month_arr[3];
	// 	if($last_record){
	// 		if($month == date('m')){
	// 	 		$new_number = $number+1;
	// 		}
	...
	// 		$new_number=1;
	//  	}
	// 	return date('y').date('m').$new_number;
	// }
										...
										<div class="col-md-9" style="padding-left:0">
											<div class="col-md-8">
												...
												<div class="form-group col-md-4 col-xs-12 no-gutter" style="padding-left: 0PX">
													...
													<span class="walk_in_text_show" style="margin-left: 8px;"></span>
												</div>
											...
											<tr class="pq_row">
												...
												<td>
													<div class="input-group table_inputs">
														<input readonly type="number" value="" onkeyup="calculate_quantity(this)" name="quantity[]" class="form-control check_quantity quantity" placeholder="Quantity">
	...
	function ChangeSubItemModalNAmes(modal_no,sr){
		...
		function QuantityAdd(val_this,modal_no) {
			...
			/*if($add_qty > $avail_qty){
				...
				$(val_this).parent().parent().find('.alterNateUnitSpan').text('Quantity Exceeding').addClass('text-danger');
				var quantity = 0;
				...
				if(alternate_unit !='' && conversion !=0){
					conversionToAlternateUnit = parseFloat($(val_this).val())/parseFloat(conversion);
					conversionToAlternateUnit = conversionToAlternateUnit.toFixed(2);
					$(val_this).parent().parent().find(".alterNateUnitSpan").text("( "+conversionToAlternateUnit+" "+alternate_unit+" )").removeClass('text-danger');
				}
				$("."+modal_no).find(".SubItemTable tfoot .quantity_span .quantity_b").text(quantity);
				$("."+modal_no).find(".SubItemTable tfoot .quantity_td input[type=hidden]").val(quantity);
			/*	}*/
