<?php
	...
	if(isset($_REQUEST['command']) && $_REQUEST['command'] == 'checkSubItems'){
		...
		$db->select("Select * from sub_item where item_id = ".$item." and FIND_IN_SET(".getCompanyId().",company_id)");
	...
	if(isset($_REQUEST['command']) && $_REQUEST['command'] == 'GetWarehouse'){
		$head = intval($_REQUEST['head']);
		$subhead = intval($_REQUEST['subhead']);
		$db->select("select * from warehouse where head = ".$head." and sub_head = ".$subhead);
	...
	if(isset($_REQUEST['command']) && $_REQUEST['command'] == 'GetSections'){
		$warehouse = intval($_REQUEST['warehouse']);
		$db->select("select * from sections where warehouse_id = ".$warehouse);
		$sections = $db->fetch_all();
		$html = '<option value="0">Select Sections</option>';
	...
	if (isset($_REQUEST['command']) && $_REQUEST['command'] == 'getWalkInSubLedgers' && intval($_REQUEST['id']) > 0) {
		$db->Select("Select * from sub_ledgers where locked = 'no' and ledger_id = ".$_REQUEST['id']);
	...
	if(isset($_REQUEST['command']) && $_REQUEST['command'] == 'GetDistributers'){
		...
		$s_loc = $db->fetch_array_by_query("Select * from item_sublocation where id = ".$sub_head);
		$head = $s_loc['location_id'];
		$html = '';
		
		if($data == 'distributer'){
			$db->Select("select * from ledger where active_distributer='yes'  and FIND_IN_SET('".$sub_head."',distributer_subhead) and FIND_IN_SET(".getCompanyId().",company_id)");		
	...
	function getContractArticles($subitem,$purchaseDetail,$row){
		global $db;
		$locs = $db->fetch_array_by_query("Select * from item_sublocation where id = ".$purchaseDetail['sub_location']);
		$purchaseDetail['location'] = $locs['location_id'];
	...
	if(isset($_REQUEST['command']) && $_REQUEST['command'] == 'getArticles'){
		...
		$subloc = $db->fetch_array_by_query("Select * from item_sublocation where id = ".$sub_location);
		$location = $subloc['location_id'];
	...
	function GetSubitemQuantity($loc,$sub_loc,$sub_item_id,$article,$quality,$section,$warehoue_id,$brand,$finishing_type){
		global $db;
		$tables = array('quotation_inventory','sales_inventory','transfer_inventory');
		$quotation_q = 0;
		$sales_q = 0;
		$transfer_q = 0;
		foreach($tables as $table){
			$sql = "Select * from ".$table." where sub_item_id=".$sub_item_id;
			if($article != '' && intval($article) > 0){
				$sql .= " and article_id = ".$article;
			}
			if($quality!='' && intval($quality) > 0){
			...
			$sql .= " and location_id=".$loc." and sub_location_id=".$sub_loc." and company_id=".getCompanyId();
			$db->select($sql);
			...
			foreach($subItems_inventory as $subItem_inv ){
				...
				}else if($table == 'transfer_inventory'){
					$transfer_q += $subItem_inv['quantity'];
				}
			}
		}
		$quantity = ($quotation_q - $transfer_q )- $sales_q;
		if($quantity < 0){
			$quantity = 0;
		}
		return $quantity;
	...
	if(isset($_REQUEST['command']) && $_REQUEST['command'] == 'getQuantity'){
		$quantity = 0;
		if(isset($_REQUEST['sub_head']) && $_REQUEST['sub_head'] !=''){
			$sub_item_id = $_REQUEST['Sub_item_id'];
			$sb_row = $db->fetch_array_by_query("Select * from sub_item where id = ".$sub_item_id);
			$subhead = $_REQUEST['sub_head'];
			$subloc = $db->fetch_array_by_query("Select * from item_sublocation where id = ".$subhead);
			$head = $subloc['location_id'];
			$quality = $_REQUEST['Sub_item_quality'];
			$article = $_REQUEST['Sub_item_article'];
			$quantity = GetSubitemQuantity($head,$subhead,$sub_item_id,$article,$quality,'','',$sb_row['brand_id'],$sb_row['finishing_type']);
		}
		echo $quantity;
		exit();
	}
	...
	if(isset($_POST['purchaseInvoice'])){
		$subhead = $db->fetch_array_by_query("select * from item_sublocation where id = ".intval($_POST['cr_sublocation']));
		$_POST['cr_location'] = $subhead['location_id'];
		$arr = array();
		...
		$arr['total_discount'] = floatval($_POST['total_discount']);
		...
		if(isset($_POST['cr_warehouse']) && isset($_POST['cr_section'])){
			$arr['cr_warehouse'] = intval($_POST['cr_warehouse']);
			$arr['cr_section'] = intval($_POST['cr_section']);
		}
		...
		$arr['other_charges'] = $_POST['other_amount'];
		$arr['grand_total'] = floatval($_POST['total']);
		...
		if($receipt_id){
			...
			if($_POST['item_name']){
					...
					if($_POST['sr_no'][$i]!=0){
						...
						if(isset($_REQUEST['Sub_item_id'.$sr_no]) && $_REQUEST['Sub_item_id'.$sr_no] != ''){								
							foreach($_REQUEST['Sub_item_id'.$sr_no] as $subItemIndex => $subItemId){
								...
								if(isset($_POST['Sub_item_unit_id'.$sr_no][$subItemIndex]) && intval($_POST['Sub_item_unit_id'.$sr_no][$subItemIndex]) > 0 ){
									$receipt_inv['unit_id'] = intval($_POST['Sub_item_unit_id'.$sr_no][$subItemIndex]);
								}else {
									$receipt_inv['unit_id'] = 0;
								}
				...
				if($receipt_inventory_id){
					MakeCommissionVoucher($arr);
					$voucher_id = makePurchaseReceiptVoucher($arr);
		...
		if($result){
			...
			$obj_msg = load_class('InfoMessages');
			$imsg->setMessage('Error Occur. Please try again.', 'error');
			redirect_header(ADMIN_URL.'sale/sale-invoice.php');
		}
	}

if(isset($_REQUEST['command']) && $_REQUEST['command'] == 'approve_invoice' && isset($_REQUEST['sid']) && intval($_REQUEST['sid']) > 0){
	$arr = $db->fetch_array_by_query("Select * from sale_invoice where id = ".$_REQUEST['sid']);
	$voucher = $db->fetch_array_by_query("Select * from voucher where sale_invoice = ".$arr['id']);	
	if($voucher){
		$ary = array('status'=>'approved');
	   	$result = $db->update($_REQUEST['sid'] , $ary , 'sale_invoice' );
	}else{
		MakeCommissionVoucher($arr);
		$voucher_id = makePurchaseReceiptVoucher($arr);
		$result = makeInvoiceTransactions($voucher_id, $arr,$general_head['value'],$general_subhead['value']);
	...
	if($result){
		$obj_msg = load_class('InfoMessages');
		$imsg->setMessage('Invoice Approved Successfully');
		redirect_header(ADMIN_URL.'sale/sale-invoice.php');
	}else{
		$obj_msg = load_class('InfoMessages');
	...
	function makePurchaseReceiptVoucher($receipt_arr){
		global $db;
		$images_arr =array();
		...
		$voucher_arr['narration'] =$receipt_arr['narration'];
	...
	function MakeCommissionVoucher($receipt_arr){
		...
		if((isset($receipt_arr['total_commission']) && intval($receipt_arr['total_commission']) >0 ) && $receipt_arr['commission'] == 'yes'){
			...
			$cr_transaction['amount'] = intval($receipt_arr['total_commission']);
			...
			$cr_transaction['location_id'] = intval($receipt_arr['location_id']);
			$cr_transaction['sub_location_id'] = intval($receipt_arr['sub_location_id']);
			...
			$dr_transaction['location_id'] = intval($receipt_arr['location_id']);
			$dr_transaction['sub_location_id'] = intval($receipt_arr['sub_location_id']);
	...
	function makeInvoiceTransactions($voucher_id,$receipt_arr,$head,$subhead){
		...
		if($s_invoice_transactions && ($receipt_arr['discount_mode'] == 'enable')){
			...
			$transaction_arr['location_id'] = intval($receipt_arr['location_id']);
			$transaction_arr['sub_location_id'] = intval($receipt_arr['sub_location_id']);
		...
		if($s_invoice_transactions){
			...
			$transaction_arr['amount'] = $receipt_arr['total_amount']+$receipt_arr['total_discount'];
			$transaction_arr['company_id'] = getCompanyId();
			$transaction_arr['user_id'] = getUSerId();
			$transaction_arr['created_at'] = time();
			$transaction_arr['updated_at'] = time();
		...
		if($receipt_arr['sale_type'] == 'company'){
			$transaction_arr = array();
			$transaction_arr['ledger_id'] = 2253;
			$transaction_arr['voucher_id'] = $voucher_id;
			$transaction_arr['description']=$receipt_arr['narration'];
			$transaction_arr['type'] = 'debit';
			...
			$transaction_arr['created_at'] = time();
			$transaction_arr['updated_at'] = time();
			$transaction_arr['location_id'] = intval($receipt_arr['debit_head']);
			$transaction_arr['sub_location_id'] = intval($receipt_arr['debit_subhead']);
			$cr_transaction_id = $db->insert($transaction_arr,'transactions');
		}else{
			if($s_invoice_transactions){
	...
	function getShowSubsInfo($id,$type){
		global $db;
		$subhead = $db->fetch_array_by_query("Select * from item_sublocation where id = ".$id );
		if($subhead['check_sublocation'] == 'yes' && getCompanyId() == 5){
			return $subhead['show_'.$type];
		}else{
			return $subhead['name']; 
		}
	}
...
.fa{font-size: 16px}
.discount,#delivery_date{background: white !important;color: black !important; border:1px solid #bbbbbb !important; padding: 7px 7px !important}
...
<!-- 									<div class=" col-lg-2 col-md-2  location_div">
									...
									<div class=" col-lg-2 col-md-2 location_div">
										<div class="form-group">
											...
											<option value="">Select Sub Head</option>
												<?php
												$db->select("Select * from item_sublocation where FIND_IN_SET(".getCompanyId().",company_id)");
												...
												?>
													<option  <?php echo $select; ?> value="<?php echo $sublocationResult['id'] ?>" ><?php echo getShowSubsInfo($sublocationResult['id'],'name'); ?></option>
												<?php }?>
											</select>
										</div>
									</div>
									<div class="col-lg-2 col-md-2 no-gutter">
										<div class="col-lg-2 col-md-2">
											<label><input type="checkbox" onclick="<?php if(isset($_REQUEST) && $_REQUEST['agent']!=''){?> return false <?php }else{?> EnableCommission(this)<?php } ?>" name="commission_check" class="commission_check" value="yes" <?php if(isset($_REQUEST) && $_REQUEST['agent']!=''){ echo "checked"; }?>></label>
										</div>
										<div class="col-lg-10 col-md-10 sales_person_div">
											<div class="form-group">
												<label> Sales Person </label>
												<select <?php if(isset($_REQUEST) && $_REQUEST['agent']!=''){ }else{?> disabled <?php } ?> class="form-control chosen-transporter sales_person" name="sales_person" required="true" onchange="checkWalkInCustomer(this)">
													<?php
								...
								<div class="col-sm-12 no-gutter">
										<div class="col-md-9" style="padding-left:0">
											<div class="col-md-2 ">
												...
												<div class="form-group">
													...
													<select name="sale_type" onchange="CompanyTransactions(this)" class="form-control chosen-select sale_type" id="c-type" >
														<option <?php if($pi_row['sale_type'] == 'retail_sale'){ echo "selected";}?> value="customer">Retail</option>
														<option <?php if($pi_row['sale_type'] == 'whole_sale'){ echo "selected";}?> value="whole_sale">Whole Sale</option>
														<option <?php if($pi_row['sale_type'] == 'company_sale'){ echo "selected";}?> value="company">Company</option>
														<option <?php if($pi_row['sale_type'] == 'distributer'){ echo "selected";}?> value="distributer">Distributer</option>
														<option <?php if($pi_row['sale_type'] == 'project'){ echo "selected";}?> value="project">Project</option>
														<option <?php if($pi_row['sale_type'] == 'agent'){ echo "selected";}?> value="agent">Agent</option>
													</select>
												</div>
											</div>
											<div class="col-md-10 customer_div" <?php if($pi_row['sale_type'] == 'company_sale'){ ?>style="display: none"<?php } ?>>
												...
												<div class="col-md-4 walk_in_button_show" <?php if($pi_row['sub_ledger_id'] <= 0 || empty($pi_row)){ ?> style="display:none" <?php } ?> >
													...
													<div class="input-group">
														<select class="chosen-select walk_in" onchange="getBalance(this,'subledger')" name="walk_in">
															...
															?>
															<option data-credit = "<?php echo  $transporter['credit_limit_acitve']; ?>" data-cvalue = "<?php echo  $s_ledger['credit_limit']; ?>" <?php if($s_ledger['id'] == $pi_row['sub_ledger_id']){ echo "selected";} ?> value="<?php echo $s_ledger['id']; ?>"><?php echo $s_ledger['name']; ?></option>
															<?php 
															}
															?>
														</select>
												...
												<div class="db_balance">
													<?php
														if(isset($pi_row)){
															if(intval($pi_row['sub_ledger_id']) > 0){
													...
													?>
													<input type="hidden" class="ledger_balance" value="<?php echo $balance['amount'].' '.$balance['type']; ?>" >
												</div>
											</div>
											...
											<div class="col-md-10 company_sale_div <?php if($pi_row['sale_type'] != 'company_sale'){ ?>hidden<?php } ?>  ">	
												<h4 style="border-bottom: 3px solid #dd4b39;width:fit-content;"> Debit : </h4>
												<div class="col-md-2 no-gutter">
													<div class="form-group col-xs-12 no-gutter" style="padding-left: 0PX">
														...
														<select class="form-control chosen-select" name="dr_project" required="true">
															<?php
															$purchase_ac = $db->fetch_array_by_query("Select * from ledger where id=2253");
															$sample_tl = $db->fetch_array_by_query("Select * from ledger where id=4089");
															$tl_breakage = $db->fetch_array_by_query("Select * from ledger where id=4396");
															?>
															<option selected value="<?php echo $purchase_ac['id']; ?>"> <?php echo $purchase_ac['name']; ?></option>
															<option  value="<?php echo $sample_tl['id']; ?>"> <?php echo $sample_tl['name']; ?></option>
															<option  value="<?php echo $tl_breakage['id']; ?>"> <?php echo $tl_breakage['name']; ?></option>
														</select>
													</div>
												</div>
												<div class="single-party-div col-md-5">
													<div class="form-group col-md-6 col-xs-6">
														...
														<select class="form-control chosen-select debit_head" onchange="getDebitSubLoc(this)" name="debit_head" required="true">
															...
															foreach ($locs as $loc){
															?>
															<option <?php if($pi_row['debit_head'] == $loc['id']){echo "selected";} ?> value="<?php echo $loc['id']; ?>" <?php echo $select;?>> <?php echo $loc['name']; ?></option>
															<?php } ?>
														</select>
													</div>
													<div class="form-group col-md-6 col-xs-6 no-gutter">
														<label> Sub Head </label>
														<select class="form-control chosen-select debit_subhead" onchange="GetWarehouse(this)" name="debit_subhead" required="true">
															...
															?>
															<option <?php if($pi_row['debit_subhead'] == $sloc['id']){echo "selected";} ?> value="<?php echo $sloc['id']; ?>" <?php echo $select;?>> <?php echo $sloc['name']; ?></option>
															<?php } ?>
														</select>
													</div>
												</div>
												<div class="single-party-div col-md-5">
													<div class="form-group col-md-6 col-xs-6">
														...
														<select class="form-control chosen-select credit_warehouse" onchange="GetSections(this)" name="cr_warehouse" required="true">
															<option value="0" selected=""> Select Warehouse </option>
															<?php
															$db->select('select * from warehouse where FIND_IN_SET('.getCompanyId().',company_id) order by id desc');
															$warehouses = $db->fetch_all();
															foreach ($warehouses as $warehouse){
															?>
															<option <?php if($pi_row['debit_head'] == $warehouse['id']){echo "selected";} ?> value="<?php echo $warehouse['id']; ?>" <?php echo $select;?>> <?php echo $warehouse['name']; ?></option>
															<?php } ?>
														</select>
													</div>
													<div class="form-group col-md-6 col-xs-6 no-gutter">
														<label> Section </label>
														<select class="form-control chosen-select credit_section" name="cr_section" required="true">
										...
										<!-- </div> -->
										<div class="col-sm-3 <?php if(!empty($pi_row) || !check_permission($auth_id,'sale-invoice.php?discount',false)){echo 'hidden';} ?> " style="float:right">
											...
											<div class="col-md-12">
												...
												<div class="discount_with_vehicle col-md-6 no-gutter">
													...
													<label class="radio-inline"><input <?php if($pi_row['discount_mode'] == 'enable'){echo 'checked';} ?> type="radio" value="enable" id="enable_dis" name="discount_mode" > Enable</label>
													<br>
													<label class="radio-inline"><input <?php if($pi_row['discount_mode'] == 'disable'){echo 'checked';}else if(empty($pi_row)){echo "checked";} ?> type="radio" name="discount_mode" id="disable_dis" value="disable" > Disabled</label>
												</div>
										...
										<table class="table table-striped pq_table">
											<thead>
												<tr class="title_bg">
													...
													<th colspan="<?php if($pi_row['discount_mode']=="enable"){ echo '3';}else if($pi_row['discount_mode']=="disable" || empty($pi_row)){echo "2";} ?>" class="border-b0 rate_td">Rate</th>
												...
												<tr  class="title_bg">
													...
													<th class="<?php if($pi_row['discount_mode']=="disable"){ echo 'hidden';}else if(empty($pi_row)){echo "hidden";} ?>  discount_heading border-b0">Discount</th>
											...
											<tr class="pq_row">
												...
												<input type="hidden" class="alternative_unit_id" value="<?php echo $alternate_unit['id'];?>">
												<input type="hidden" class="main_item_unit" value="<?php echo $itemSymbol['name'];?>">
												<input type="hidden" class="main_item_unit_id" value="<?php echo $itemSymbol['id'];?>">
												<input type="hidden" class="c_quantity" value="<?php echo $item['c_quantity'];?>">
																			...
																			<div class="table-responsive">
																				<table class="table table-striped table-bordered SubItemTable">
																					<thead>
																						<tr>
																							<th>Code</th>
																							<th class="article_td"  <?php if($checkItem['article'] == 'no'){ echo 'style="display:none"';}?>>Article</th>
																							<th class="quality_td" style="display:none" <?php if($checkItem['quality'] == 'no'){ echo '';}?>>Quality</th>
																							<th class="color_td" <?php if($checkItem['color'] == 'no'){ echo'style="display:none"';}?> >Color</th>
																							<?php if(intval($pi_row['purchase_id']) >0){ ?>
																							<th class="warehouse_td" <?php if($checkItem['warehouse'] == 'no'){ echo'style="display:none"';}?> >Warehouse
																							</th>
																							<th class="section_td" <?php if($checkItem['section'] == 'no'){ echo'style="display:none"';}?> >Section</th>
																							<?php } ?>
																							<th style="width: 28%" colspan="2">Quantity</th>
																							<th colspan="3">Available Quantity</th>
																							<th>Images</th>
																						</tr>
																						<tr>
																							<th></th>		
																							<th class="article_td"  <?php if($checkItem['article'] == 'no'){ echo 'style="display:none"';}?>></th>
																							<th class="quality_td" style="display:none" <?php if($checkItem['quality'] == 'no'){ echo '';}?>></th>
																							<th  class="color_td" <?php if($checkItem['color'] == 'no'){ echo'style="display:none"';}?>></th>
																							<th style="width: 20%" >Value</th>
																							<th style="width: 14%" >Unit</th>
																							<th class="hidden">Ton</th>
																							<th class="qty_symbol"><?php echo $itemSymbol['name'].'s'; ?></th>
																							<th>Boxes</th>
																							<th>Availability</th>
																							<th></th>
																						</tr>
																					...
																					<tbody class="SubItemBody">
																						...
																						<tr class="SubItemRow">
																							...
																							<td class="article_td"  <?php if($checkItem['article'] == 'no'){ echo 'style="display:none"';}?>>
																								<div class="form-group">
																									<?php
																							...
																							<td class="quality_td" style="display:none" <?php if($checkItem['quality'] == 'no'){echo '';}?>>
																								<div class="form-group">
																									<?php
																							...
																							<td class="color_td" <?php if($checkItem['color'] == 'no'){echo 'style="display:none"';}?>>
																								<div class="form-group">
																									<?php
																						...
																						<td>
																								...
																								<div class="input-group table-inputs">
																									...
																									<span class="pd_right input-group-addon no-gutter">
																										<select name="Sub_item_unit_id<?php echo $sr_no; ?>[]" onchange="QuantityValue('<?php echo 'SubItemModal'.$sr_no;?>',this)"class="showQ form-control" style="width: auto">
																											<?php 
																												$db->Select("Select * from item_unit where FIND_IN_SET(".getCompanyId().",company_id)"); 
																												$units = $db->fetch_all();
																												foreach($units as $unit){
																													$select = '';

																													if(intval($subItem_inventory['unit_id']) > 0){
																														if($subItem_inventory['unit_id'] == $unit['id']){
																															$select = 'selected';
																														}
																												...
																												?>
																												<option <?php echo $select; ?> value="<?php echo $unit['id']; ?>"><?php echo $unit['name']; ?></option>
																												<?php } ?>
																										</select>
																									</span>
																								</div>
																							</td>
																							<td class="alternate_td">
																								<?php
																									if($checkItem['c_quantity'] == 0 || $checkItem['c_quantity'] ==''){
																										$checkItem['c_quantity'] = 1;
																									}
																									$checkItem['conversion'] = ($checkItem['conversion'])/$checkItem['c_quantity'];
																									if(($subItem_inventory['unit_id'] == $itemSymbol['id']) && intval($checkItem['alternative_unit_id'])>0){
																										echo $subItem_inventory['value'].' '.$itemSymbol['symbol'].' ('.round($subItem_inventory['value']/$checkItem['conversion'],2).' '.$alternate_unit['symbol'].')';
																									}else if(($subItem_inventory['unit_id'] == $alternate_unit['id']) && intval($checkItem['alternative_unit_id'])>0){
																										echo $subItem_inventory['value'].' '.$alternate_unit['symbol'].' ('.round($subItem_inventory['value']*$checkItem['conversion'],2).' '.$itemSymbol['symbol'].')';
																									}else{
																										echo $subItem_inventory['inv_quantity'].' '.$itemSymbol['symbol'];
																									}
																								?>
																							</td>
																							<td class="quantity_td hidden">
																								<div class=" input-group">
																							...
																							<td class="number_td hidden"> 
																								<div class="input-group">
																								<input name="Sub_item_tons<?php echo $sr_no; ?>[]" type="number" readonly value="<?php if($subItem_inventory['tons'] > 0){echo $subItem_inventory['tons'];} ?>" class="form-control inv_tons">
																								</div>
																							</td>
																							<td>
																							<?php
																								...
																								$avail_q = GetSubitemQuantity( $subItem_inventory['location_id'],$subItem_inventory['sub_location_id'],$subItem_inventory['sub_item_id'],$subItem_inventory['article_id'],$subItem_inventory['quality_id'],$sec_id,$warehouse_id,$subItemrow['brand_id'],$subItemrow['finishing_type']);
																							?>
																								<span style="font-weight: bold" class="available_quantity"><?php echo $avail_q;?>
																								</span>
																							</td>
																							<td>
																								<span style="font-weight: bold" class="available_quantity_boxes"><?php if(isset($checkItem['conversion']) and intval($checkItem['conversion']) > 0){ echo round($avail_q/$checkItem['conversion'],2);} else{ echo ' - ' ; } ?></span>
																							</td>
																							<td>
																					...
																					<tfoot>
																						<tr>
																							<td></td>
																							<td class="article_td" <?php if($checkItem['article'] == 'no'){ echo 'style="display:none"';}?>></td>
																							<td class="quality_td" style="display:none" <?php if($checkItem['quality'] == 'no'){ echo '';}?>></td>
																							<td class="color_td" <?php if($checkItem['color'] == 'no'){ echo'style="display:none"';}?> ></td>
																							<td>Total</td>
																							...
																							<td class="boxes_td hidden"><span class="boxes_span"><b class="boxes_b"><?php echo $inv_boxes; ?></b></span> <input type="hidden" value="<?php echo $inv_boxes; ?>" class="total_boxes"></td>
														...
														<div class="form-group">
															<select class="form-control chosen-select item_brand" name="brand[]" placeholder=" Enter Item Name" onchange="getQuality(this)">
																...
																<?php
																	...
																	$brands = $db->fetch_all();
																	foreach ($brands as $brand){?>
																		<option <?php if($purchaseDetail['brand'] == $brand['id']){ echo "selected";} ?> id='<?php echo ($brand['id']) ?>' value='<?php echo ($brand['id']) ?>'> <?php echo $brand['name'] ?> </option>
																<?php } ?>
															</select>
														</div>
													</div>
															...
															<select class="form-control chosen-select item_quality" name="quality[]" placeholder=" Enter Item Name" onchange="chosenValue(this)">
																...
																<?php
																	...
																	foreach ($qualities as $quality){
																			$qualiti = $db->fetch_array_by_query("select * from item_quality where id=".$quality['q_id']);
																?>
																		<option <?php if($purchaseDetail['quality'] == $qualiti['id']){ echo "selected";} ?> id='<?php echo ($quality['q_id']) ?>' value='<?php echo ($quality['q_id']) ?>'> <?php echo $qualiti['name'] ?> </option>
																<?php } ?>
															</select>
														</div>
													</div>
												...
												<td>
													<div class="input-group table_inputs">
														...
														<span class="input-group-addon"><b class="qty_symbol_per"><?php if($itemSymbol['symbol']!=''){
												...
												<td>
													<div class="input-group table_inputs">
														...
														<span class="input-group-addon"><b class="qty_symbol_per"><?php if($alternate_unit['symbol']!=''){
																echo $alternate_unit['symbol'];
															}else{
																echo "unit";
															} ?></b></span>
													</div>
													...
													<div class="input-group table_inputs">
														...
														<span style="padding: 0px;" class="pd_right input-group-addon">
															<select onchange="calculate(this)" class="discount_type form-control" name="balance_category[]" style="width: auto;">
																<option <?php if($purchaseDetail['balance_category']=="Rs"){ echo "selected";} ?> value="Rs">No</option>
																<?php /*<option <?php if($purchaseDetail['balance_category']=="%"){ echo "selected";} ?>  value="%">%</option>*/ ?>
															</select>
														</span>
													</div>
											...
											<?php } ?>
												<td>
													...
													<div class="hidden_disc_amt"> <?php if ($pi_row['discount_mode']=='enable' && $purchaseDetail['discount_mode']=="enable"){ echo 'Discount: '.$disc_amt; }?></div>
											...
											<?php }
												...
												<tr class="pq_row">
													...
													<input type="hidden" class="alternative_unit_id">
													<input type="hidden" class="main_item_unit">
													<input type="hidden" class="main_item_unit_id">
													<input type="hidden" class="c_quantity">
													...
													<td class="group_chosen">
														...
														<div style="width: 100% !important" class="input-group">
															<select name="item_group[]" onchange="group_item(this)" class="chosen-select form-control item_group">
																<option> Select Item Group</option>
																<?php
																	$db->select('SELECT * FROM item_group where FIND_IN_SET('.getCompanyId().',company_id)');
																		$group_results = $db->fetch_all();
																		foreach ($group_results as $group_result){?>
												...
												<td>
													...
													<td>
														<div class="input-group">
															...
															<span class="input-group-addon"><b class="qty_symbol">unit</b></span>
														</div>
													</td>
													<td>
														<div class="input-group">
															<input readonly type="number" name="boxes[]" class="form-control boxes" placeholder="Boxes">
															<span class="input-group-addon"><b class="alt_qty_symbol">unit</b></span>
														</div>
													</td>
													<td class="hidden">
														<div class="form-group">
											...
											<tfoot>
												<tr class="footer_bg">
													...
													<td></td>
													<td class="discount_heading <?php if($pi_row['discount_mode']=="disable"){ echo 'hidden';}else if(empty($pi_row)){echo "hidden";}?> "><span id="total_discount_txt">0</span></td>
													<td></td>
									...
									<table class="table table-striped table-bordered SubItemTable">
										<thead>
											<tr>
												...
												<th colspan="2" style="width: 28%">Quantity</th>
												<th colspan="3">Available Quantity</th>
												<th>Image</th>
												<th>Action</th>
											</tr>
												...
												<td>
													<div class="input-group qty-span">
														...
														<span class="input-group-addon no-gutter">
															<select class="showQ form-control " style="width: auto">
																<?php 
																$db->Select("Select * from item_unit where FIND_IN_SET(".getCompanyId().",company_id)"); 
																$units = $db->fetch_all();
																foreach($units as $unit){
										...
										<tfoot>
											<tr>
												...
												<td class="boxes_td hidden"><span class="boxes_span"><b class="boxes_b">0</b></span> <input type="hidden" class="total_boxes"></td>
	...
	<script type="text/javascript">
		...
		<?php if(isset($_REQUEST['so_id']) && intval($_REQUEST['so_id']) > 0){ ?> 
			getTotalCredit();
	 	<?php } ?>
		...
		function chosenValue(val_this){
			...
			if(project_id=="0" || project_id== null || isNaN(project_id)){
				...
				data: {data_id},
					...
					checkSubItems(val_this,project_id);
					...
					units.item_unit.forEach(function(unit){
						...
						$(val_this).parent().parent().parent().parent().find('.main_item_unit_id').val(unit.id);
					...
					units.alternative_unit.al_unit.forEach(function(al_unit) {
						...
						$(val_this).parent().parent().parent().parent().find('.alt_qty_symbol').html(al_unit.name);
						$(val_this).parent().parent().parent().parent().find('.alternative_unit').val(al_unit.name);
						$(val_this).parent().parent().parent().parent().find('.alternative_unit_id').val(al_unit.id);
					});
		...
		function CheckSalesPersonContract(val_this){
			...
			$.ajax({
				...
				type : 'post',
				data : {item_id,location_id,project_id,sales_person},
		...
		function checkSubItems(val_this,project_id){
				...
				success : function(result){
					...
					if(result == 'yes'){
						...
						$.ajax({
							...
							type: "POST",
							data: {'item':'Add_item',item_id,brand_id,finishing_type,quality_id,project_id},
						}).done(function(result) {
							res = $.parseJSON(result);
							if(res.sub_item == 'yes'){
								// for item quality
	...
	function ShowSubItemModal(val_this) {
		...
		var item_id =  $(val_this).parent().parent().parent().parent().find(".check_item").val();
		var main_item_unit_id = $(val_this).parent().parent().parent().parent().find(".main_item_unit_id").val();
		...
		if($('.SubItemModal'+sr_no).html()!=undefined){
			...
			$('.SubItemModal'+sr_no).find('.showQ').val(main_item_unit_id);
		...
		function getArticles(val_this,type){
			...
			$.ajax({
				url:'add-sale-invoice.php',
				data : {code,item,brand,project_id,sale_type,customer,quality,finishing_type,type,command:'getArticles'},
				type : 'POST',
				success : function(data){
		...
		function GetContractSubitems(val_this){
			...
			$.ajax({
				url : '<?php echo ADMIN_URL?>ajax/get-item-specifications.php',
				type : 'post',
				data : {sale_type,item_id,brand,quality,customer,project_id,finishing_type,location_id,command:'getContractSubitems'},
	...
	function getQuality(val_this){
		...
		$.ajax({
			url : 'add-sale-invoice.php',
			type : 'post',
			data : {item,brand,project_id,location_id,finishing_type,command:'getQualities'},
			success : function(data){
	...
	function ChangeSubItemModalNAmes(modal_no,sr){
		...
		$("."+modal_no+" .showQ").attr("name","Sub_item_unit_id"+sr+'[]');
		...
		function checkContract(val_this){			
			...
			method: 'POST',
			data: {item_id,project_id,brand,sale_type,sales_person,quality,finishing_type,customer}
			}).done(function(result){
				res = $.parseJSON(result);
		...
		function getSubwareHouseITemQuantity(val_this){
			...
			var alt_unit_id = $(val_this).closest('.brand_qual_modal').parent().find(".alternative_unit_id").val();
			...
			$.ajax({
				url : 'add-sale-invoice.php',
				type : 'post',
				data : {sub_head,Sub_item_id,Sub_item_article,Sub_item_quality,command:'getQuantity'},
				success : function(result){
		...
		function EnableCommission(this_val){
			...
			$(".sales_person_div select").chosen({width:'100%'})
		}
		...
		function AddSubItemRow(val_this){
			...
			var showq = $(val_this).parent().parent().find(".showQ").val();
			...
			$(val_this).parent().parent().parent().find("tr:last .showQ").val(showq);
		...
		function checkWalkInCustomer(val_this) {
			...
			if ($(val_this).find("option:selected").attr("data-active-walk-in")=='yes') {
				...
				$('.walk_in_button_show .walk_in').chosen({width:'100%'});
		...
		function getBalance(val,type){
			...
			$.ajax({
				url : 'add-sale-invoice.php',
				type: 'post',
				data : {type,ledger_id,subledger,command:'getBalance'},
	...
	function getTotalCredit(){
		...
		function QuantityValue(modal,val_this){
			var unit = $(val_this).parent().parent().find(".showQ").val();
			var txt_unit = $(val_this).parent().parent().find(".showQ option:selected").text();
			...
			var alternative_unit = $('.'+modal).parent().parent().find(".alternative_unit").val();
			var alt_unit_id = $('.'+modal).parent().parent().find(".alternative_unit_id").val();
			var main_item_unit_id = $('.'+modal).parent().parent().find(".main_item_unit_id").val();
			var main_item_unit = $('.'+modal).parent().parent().find(".main_item_unit").val();
			console.log(alt_unit_id);
			if( (unit == alt_unit_id) && alt_unit_id!=''){
				AlterNativeUnitToQuntity(quantity,val_this,alt_unit,main_item_unit,alternative_unit)
			}else if( (unit == main_item_unit_id) && alt_unit_id!=''){
				QuntityToAlterNativeUnit(quantity,val_this,alt_unit,alternative_unit,main_item_unit);
			}else{
				$(val_this).closest("tr").find(".boxes_td input").val(0);
				$(val_this).closest("tr").find(".alternate_td").text(quantity+ ' '+main_item_unit);
			...
			$("."+modal).find('.SubItemTable tbody tr').each(function(){
				q += parseFloat($(this).find(".inv_quantity").val());
				boxes += parseFloat($(this).find(".inv_boxes").val());
				tons += parseFloat($(this).find(".inv_tons").val());
			});
			$("."+modal).find(".SubItemTable tfoot .quantity_span .quantity_b").text(q);
			$("."+modal).find(".SubItemTable tfoot .quantity_td input[type=hidden]").val(q);
		...
		function QuntityToAlterNativeUnit(q,val_this,alt_unit,alternative_unit,showUnit){
			quantity = parseFloat(q).toFixed(2);
			var alternateQuantity = (parseFloat(quantity)/parseFloat(alt_unit)).toFixed(2);
			$(val_this).closest("tr").find(".boxes_td input").val(alternateQuantity);
			$(val_this).closest("tr").find(".alternate_td").text(quantity +' '+ showUnit + ' (' +alternateQuantity+' '+alternative_unit+')');
			$(val_this).closest("tr").find(".quantity_td input").val(quantity);
		}

		function AlterNativeUnitToQuntity(q,val_this,alt_unit,alternative_unit,showUnit){
			quantity = parseFloat(q).toFixed(2);
			var alternateQuantity = (parseFloat(quantity)*parseFloat(alt_unit)).toFixed(2);
			$(val_this).closest("tr").find(".boxes_td input").val(quantity);
			$(val_this).closest("tr").find(".alternate_td").text(quantity +' '+ showUnit + ' (' +alternateQuantity+' '+alternative_unit+')');
			$(val_this).closest("tr").find(".quantity_td input").val(alternateQuantity);
		}
		...
		function getDebitSubLoc(val_this){
			...
			.done(function(data) {
				...
				$('.debit_subhead').chosen({width:'100%'});
		...
		function GetCustomers(){
			$.ajax({
				url : 'add-sale-invoice.php',
				datatype:'post',
				data : {data:$(".sale_type").val(),order_customer:$(".order_customer").val(),sub_head:$(".sub_location").val(),command:"GetDistributers"},
		...
		function getWalkInCustomers(id){
			$.ajax({
				url : 'add-sale-invoice.php',
				type : 'post',
				data : {id,command : 'getWalkInSubLedgers'},
		...
		function getSubITemRate(val,val_this){
			...
			var article_id = $(val).parent().find(".SubItemArticle ").val();
			var sale_type = $(".sale_type ").val();
			...
			$.ajax({
				...
				data : {subitem_id,article_id,sale_type,item_id,brand,quality,sub_ledger,customer_id,finishing_type,project_id,location_id,command:'getSubITemRate'},
				type: 'post',
		...
		function GetWarehouse(val_this){
			var head = $(".debit_head").val();
			var subhead = $(val_this).val();
			$.ajax({
				url:'add-sale-invoice.php',
				data:{head,subhead,command:'GetWarehouse'},
				type:'post',
				success : function(warehouses){
					$(".credit_warehouse").html(warehouses).trigger("chosen:updated");
				}
			});
		}

		function GetSections(val_this){
			var warehouse = $(val_this).val();
			$.ajax({
				url:'add-sale-invoice.php',
				data:{warehouse,command:'GetSections'},
				type:'post',
				success : function(sections){
					$(".credit_section").html(sections).trigger("chosen:updated");
				}
			});
		}
	</script>
